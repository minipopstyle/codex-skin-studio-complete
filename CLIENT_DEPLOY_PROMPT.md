# Codex Skin Studio Complete｜客户部署提示词

使用方法：把完整的 `Codex Skin Studio Complete.zip`、客户的主题视频（或图片）和下面整段提示词，一起发送给客户自己的 macOS Codex。不要提前删除 ZIP 内的隐藏目录。

```text
你是我这台 Mac 上的 Codex 主题部署工程师。请直接完成部署、自检和实机验收，不要只给教程，也不要在没有验证证据时声称完成。

我随消息上传了“Codex Skin Studio Complete.zip”。如果还有视频附件，请优先把视频作为我的首页皮肤；如果只有图片，则应用静态主题；如果没有素材，使用项目自带的示例主题。

最终目标：
- 为官方 macOS Codex Desktop 应用一套可持续使用、可更换素材、可验证、可一键恢复的主题；
- 视频皮肤默认显示同帧海报，双击首页背景才播放，再次双击停在当前帧，第三次从该帧续播；
...
3. 将隐藏引擎记为 `<ENGINE>`，先完整阅读：
   - `<ENGINE>/README.md`
   - `<ENGINE>/SKILL.md`
   - `<ENGINE>/skills/codex-video-skin/SKILL.md`
   - `<ENGINE>/skills/codex-web-skin-builder/SKILL.md`
   - `<ENGINE>/使用说明.md`
...
4. 确认官方 Codex 至少运行过一次，且 `~/.codex/config.toml` 已存在。运行：
   ...
   完整项目应被安装到 `~/.codex/codex-dream-skin-studio`，视频 Skill 应被安装到 `~/.codex/skills/codex-video-skin`，网页皮肤 Skill 应被安装到 `~/.codex/skills/codex-web-skin-builder`，并生成桌面启动、定制、验证和恢复入口。

5. 如果我上传了视频，先运行：
   `~/.codex/skills/codex-video-skin/scripts/apply_video_skin.sh --check`
   如果缺少 `ffmpeg` / `ffprobe`，必须先征求我同意，再使用可信的 Homebrew `ffmpeg` 包；不得静默安装依赖。预检通过后运行：
   `~/.codex/skills/codex-video-skin/scripts/apply_video_skin.sh "<视频绝对路径>" "我的 Codex 视频皮肤"`
   如果我只上传了主题图片，使用安装后的脚本处理素材：
   `~/.codex/codex-dream-skin-studio/scripts/customize-theme-macos.sh --image "<图片绝对路径>" --name "我的 Codex 皮肤" --no-apply`
...
7. 启动后必须运行：
   ...
   验证器必须真实返回 `pass: true`。如果是视频皮肤，还要验收海报与视频裁切一致、人物居中、标题左对齐，以及播放→暂停当前帧→原帧续播。随后检查一个正常任务页面，确认正文、菜单、侧栏和输入框仍清晰可用，并保存任务页截图。
...
8. 检查桌面已存在以下四个入口：
   ...
   同时检查 `~/.codex/skills/codex-video-skin/SKILL.md` 和其自动应用脚本存在且可执行。
   也检查 `~/.codex/skills/codex-web-skin-builder/SKILL.md` 和其验证脚本存在。
...
最终向我汇报：
- Codex Skin Studio Complete 版本与官方 Codex 版本；
- 实际使用的主题名和素材文件；
- 视频转码后大小、焦点坐标与播放/暂停/续播验收结果（如适用）；

((cssText, artDataUrl, themeConfig, videoDataUrl) => {
  const STATE_KEY = "__CODEX_DREAM_SKIN_STATE__";
  const DISABLED_KEY = "__CODEX_DREAM_SKIN_DISABLED__";
  const STYLE_ID = "codex-dream-skin-style";
  const CHROME_ID = "codex-dream-skin-chrome";
  const SHELL_ATTR = "data-dream-shell";
  const VIDEO_ID = "codex-dream-video";
  document.getElementById(VIDEO_ID)?.remove();
  document.querySelectorAll(".dream-skin-velorah-hero").forEach((node) => node.remove());
  document.querySelectorAll(".dream-skin-mainframe-hero").forEach((node) => node.remove());
  document.documentElement?.classList.remove("dream-skin-video-active");
  document.documentElement?.classList.remove("dream-skin-video-ready");
  const ART_ATTRS = [
    "data-dream-art-wide", "data-dream-art-safe", "data-dream-task-mode",
    "data-dream-art-safe-area", "data-dream-art-task-mode", "data-dream-art-aspect",
    "data-dream-art-ready", "data-dream-homepage-style",
  ];
  const VERSION = __DREAM_SKIN_VERSION_JSON__;
  const STYLE_REVISION = __DREAM_SKIN_STYLE_REVISION_JSON__;
  const THEME = themeConfig && typeof themeConfig === "object" ? themeConfig : {};
  const HOME_COPY = {
    title: THEME.heroTitle || "Make ChatGPT Great Again",
    tagline: THEME.tagline || THEME.heroBody || "It's time to crown the new king, Tibo.",
  };
  ...
  const blobUrl = (dataUrl, fallbackMime) => {
    if (!dataUrl) return "";
  ...
  const artUrl = blobUrl(artDataUrl, "image/png");
  const videoUrl = blobUrl(videoDataUrl, "video/mp4");
  videoDataUrl = "";
  ...
  if (previous?.mainframeMousemoveHandler) window.removeEventListener("mousemove", previous.mainframeMousemoveHandler);
  if (previous?.mediaHandler && previous?.mediaQuery) {
    try { previous.mediaQuery.removeEventListener("change", previous.mediaHandler); } catch {}
  }
  if (previous?.doubleClickHandler) document.removeEventListener("dblclick", previous.doubleClickHandler, true);
  ...
  const applyTheme = (root, shell) => {
    ...
    setStyleProperty(root, "--dream-skin-tagline", cssString(HOME_COPY.tagline));
    ...
    setAttribute(root, "data-dream-homepage-style", ["velorah", "mainframe"].includes(THEME.homepageStyle)
      ? THEME.homepageStyle : "default");
  };
  ...
  let velorahHero = null;
  let mainframeHero = null;
  let mainframeTypeTimer = null;
  let mainframeTypeInterval = null;
  let mainframeSubtitleTimer = null;
  let mainframeSubtitleInterval = null;
  let mainframePrevX = null;
  let mainframeTargetTime = 0;
  let mainframeSeeking = false;
  let observedShellMain = null;
  let resizeObserver = null;
  let homeTitleSource = null;
  let homeTitleNodes = [];

  const syncHomeCopy = (home) => {
    const source = home?.querySelector('[data-feature="game-source"]') || null;
    if (source !== homeTitleSource || !homeTitleNodes[0]?.isConnected) {
      homeTitleSource = source;
      homeTitleNodes = [];
      if (source) {
        const walker = document.createTreeWalker(source, NodeFilter.SHOW_TEXT);
        let node;
        while ((node = walker.nextNode())) {
          if (node.textContent.trim() && !node.parentElement?.closest("button")) {
            homeTitleNodes.push(node);
          }
        }
      }
    }
    if (!homeTitleNodes.length) return;
    setTextContent(homeTitleNodes[0], HOME_COPY.title);
    for (const node of homeTitleNodes.slice(1)) setTextContent(node, "");
  };

  const syncVelorahHero = (chrome, home) => {
    if (THEME.homepageStyle !== "velorah" || !home) {
      velorahHero?.remove();
      velorahHero = null;
      return;
    }
    if (!velorahHero?.isConnected || velorahHero.parentElement !== chrome) {
      velorahHero?.remove();
      velorahHero = document.createElement("section");
      velorahHero.className = "dream-skin-velorah-hero";
      velorahHero.setAttribute("aria-hidden", "true");
      velorahHero.innerHTML = "<h1></h1><p></p><span></span>";
      chrome.appendChild(velorahHero);
    }
    const title = velorahHero.querySelector("h1");
    const mutedParts = String(THEME.heroMuted || "").split("|").map((part) => part.trim()).filter(Boolean);
    if (title && title.dataset.source !== HOME_COPY.title) {
      title.replaceChildren();
      let cursor = 0;
      for (const muted of mutedParts) {
        const index = HOME_COPY.title.toLocaleLowerCase().indexOf(muted.toLocaleLowerCase(), cursor);
        if (index < 0) continue;
        title.append(document.createTextNode(HOME_COPY.title.slice(cursor, index)));
        const emphasis = document.createElement("em");
        emphasis.textContent = HOME_COPY.title.slice(index, index + muted.length);
        title.append(emphasis);
        cursor = index + muted.length;
      }
      title.append(document.createTextNode(HOME_COPY.title.slice(cursor)));
      title.dataset.source = HOME_COPY.title;
    }
    setTextContent(velorahHero.querySelector("p"), HOME_COPY.tagline);
    setTextContent(velorahHero.querySelector("span"), THEME.quote || "Begin Journey");
  };

  const syncMainframeHero = (home) => {
    if (THEME.homepageStyle !== "mainframe" || !home) {
      mainframeHero?.remove();
      mainframeHero = null;
      if (mainframeTypeTimer) clearTimeout(mainframeTypeTimer);
      if (mainframeTypeInterval) clearInterval(mainframeTypeInterval);
      if (mainframeSubtitleTimer) clearTimeout(mainframeSubtitleTimer);
      if (mainframeSubtitleInterval) clearInterval(mainframeSubtitleInterval);
      mainframeTypeTimer = null;
      mainframeTypeInterval = null;
      mainframeSubtitleTimer = null;
      mainframeSubtitleInterval = null;
      return;
    }
    if (mainframeHero?.isConnected) return;

    mainframeHero = document.createElement("section");
    mainframeHero.className = "dream-skin-mainframe-hero";
    mainframeHero.innerHTML = `
      <div class="mainframe-content">
        <p class="mainframe-intro">Hey there, meet A.R.I.A,<br>Mainframe's Adaptive Response Interface Agent</p>
        <p class="mainframe-typewriter"><span data-mainframe-typed></span><i class="mainframe-cursor" aria-hidden="true"></i></p>
        <p class="mainframe-subtitle"><span data-mainframe-subtitle-typed></span><i class="mainframe-cursor" aria-hidden="true"></i></p>
        <button class="mainframe-compose-button" type="button" aria-label="Open composer" aria-expanded="false" aria-controls="codex-mainframe-composer"><span data-mainframe-compose-label>Here We Go</span></button>
      </div>
      `;
    document.body.appendChild(mainframeHero);

    const composeButton = mainframeHero.querySelector(".mainframe-compose-button");
    const composeLabel = mainframeHero.querySelector("[data-mainframe-compose-label]");
    composeButton?.addEventListener("click", () => {
      const currentHome = document.querySelector('[role="main"].dream-skin-home');
      const composer = currentHome?.querySelector(".composer-surface-chrome") ||
        document.querySelector("main.main-surface.dream-skin-home-shell .composer-surface-chrome");
      if (!currentHome || !composer) return;
      composer.id ||= "codex-mainframe-composer";
      const open = currentHome.classList.toggle("mainframe-compose-open");
      mainframeHero?.classList.toggle("is-compose-open", open);
      composeButton.setAttribute("aria-expanded", String(open));
      composeButton.setAttribute("aria-label", open ? "Close composer" : "Open composer");
      if (composeLabel) composeLabel.textContent = open ? "Until Tomorrow" : "Here We Go";
      if (open) {
        window.setTimeout(() => {
          composer.querySelector("textarea, [contenteditable=\"true\"], input")?.focus();
        }, 0);
      }
    });

    const typed = mainframeHero.querySelector("[data-mainframe-typed]");
    const subtitleTyped = mainframeHero.querySelector("[data-mainframe-subtitle-typed]");
    const subtitleText = HOME_COPY.tagline;
    const subtitleCursor = mainframeHero.querySelector(".mainframe-subtitle .mainframe-cursor");
    const text = HOME_COPY.title;
    mainframeTypeTimer = setTimeout(() => {
      let index = 0;
      mainframeTypeInterval = setInterval(() => {
        index += 1;
        setTextContent(typed, text.slice(0, index));
        if (index >= text.length) {
          clearInterval(mainframeTypeInterval);
          mainframeTypeInterval = null;
          mainframeHero?.querySelector(".mainframe-cursor")?.classList.add("is-done");
          mainframeSubtitleTimer = setTimeout(() => {
            let subtitleIndex = 0;
            mainframeSubtitleInterval = setInterval(() => {
              subtitleIndex += 1;
              setTextContent(subtitleTyped, subtitleText.slice(0, subtitleIndex));
              if (subtitleIndex >= subtitleText.length) {
                clearInterval(mainframeSubtitleInterval);
                mainframeSubtitleInterval = null;
                subtitleCursor?.classList.add("is-done");
                composeButton?.classList.add("is-ready");
              }
            }, 38);
          }, 180);
        }
  ...
  const playVideo = () => {
    ...
    document.documentElement?.classList.add("dream-skin-video-active");
    video.play().catch(stopVideo);
  };
  const syncVideo = (home) => {
    const source = videoUrl;
    if (!home || !source || matchMedia("(prefers-reduced-motion: reduce)").matches) {
      destroyVideo();
      return;
    }
    if (video?.isConnected && video.dataset.source === source) {
      videoReady = true;
      document.documentElement?.classList.add("dream-skin-video-ready");
      return;
    }
    destroyVideo();
    mainframePrevX = null;
    mainframeTargetTime = 0;
    mainframeSeeking = false;
    ...
    video.preload = THEME.homepageStyle === "mainframe" ? "auto" : "none";
    if (THEME.homepageStyle === "mainframe") video.src = videoUrl;
    video.addEventListener("seeked", () => {
      mainframeSeeking = false;
      if (THEME.homepageStyle === "mainframe" && Math.abs(video.currentTime - mainframeTargetTime) > 0.01) {
        mainframeSeeking = true;
        video.currentTime = mainframeTargetTime;
      }
    });
    Object.assign(video.style, {
      ...
      height: "100%",
      objectFit: "cover",
      objectPosition: "var(--ds-art-position)",
      pointerEvents: "none",
      opacity: "1",
      filter: "none",
    });
  ...
  const mainframeMousemoveHandler = (event) => {
    if (THEME.homepageStyle !== "mainframe" || !document.querySelector("main.main-surface.dream-skin-home-shell") || !video || !Number.isFinite(video.duration) || video.duration <= 0) return;
    const previousX = mainframePrevX ?? event.clientX;
    mainframePrevX = event.clientX;
    mainframeTargetTime = clamp(mainframeTargetTime + ((event.clientX - previousX) / window.innerWidth) * 0.8 * video.duration, 0, video.duration);
    if (!mainframeSeeking) {
      mainframeSeeking = true;
      video.currentTime = mainframeTargetTime;
    }
  ...
  const doubleClickHandler = (event) => {
    if (THEME.homepageStyle === "mainframe" || !videoReady || event.button !== 0) return;
    const target = event.target instanceof Element ? event.target : null;
    if (target?.closest('aside.app-shell-left-panel, .composer-surface-chrome, button, a, input, textarea, select, [contenteditable="true"], [role="button"]')) return;
  ...
  const syncRouteState = (shell, { layout = false } = {}) => {
    ...
    const home = [...document.querySelectorAll('[role="main"]')].find((candidate) => {
      if (!candidate.querySelector('[data-feature="game-source"]')) return false;
    ...
    syncVelorahHero(chrome, home);
    syncMainframeHero(home);
    setTextContent(chromeParts.name, THEME.name || "Codex Skin Studio");
    ...
    if (layout || created) {
      ...
      if (mainframeHero) setStyleProperty(mainframeHero, "--mainframe-shell-left", `${Math.round(shellBox.left)}px`);
    }
  ...
  const cleanup = () => {
    ...
    window.removeEventListener("mousemove", mainframeMousemoveHandler);
  ...
  const scheduleEnsure = ({ root = false, route = true, layout = false } = {}) => {
    ...
    // The app mutates its DOM frequently; a timer debounce prevents route
    // reconciliation from becoming a per-frame render loop.
    scheduler.timeout = setTimeout(flushScheduledEnsure, SCHEDULE_DELAY_MS);
  };
  ...
  window.addEventListener("mousemove", mainframeMousemoveHandler, { passive: true });

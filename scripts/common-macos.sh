...
if [ -z "${HOME:-}" ]; then
  ...
  [ -n "$HOME" ] || { printf 'Codex Skin Studio: could not resolve the current macOS home directory.\n' >&2; exit 1; }
  export HOME
fi
...
on run argv
  display notification (item 1 of argv) with title "Codex Skin Studio"
end run
APPLESCRIPT
}
...
alert_user() {
  ...
  /usr/bin/osascript - "$message" <<'APPLESCRIPT' >/dev/null 2>&1 || true
on run argv
  display alert "Codex Skin Studio" message (item 1 of argv)
end run
APPLESCRIPT
}
...
stop_recorded_injector() {
  ...
  if ! pid="$(state_field injectorPid 2>/dev/null)" || [ -z "${pid:-}" ]; then
    printf 'Skin Studio state is damaged or missing its injector PID; state was preserved.\n' >&2
    return 1
  fi
  ...
  case "$pid" in
    *[!0-9]*|??????????*)
      printf 'Recorded Skin Studio injector PID is invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  ...
  case "$saved_port" in
    ''|*[!0-9]*)
      printf 'Recorded Skin Studio injector port is missing or invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  [ "$saved_port" -ge 1024 ] && [ "$saved_port" -le 65535 ] || {
    printf 'Recorded Skin Studio injector port is out of range; state was preserved.\n' >&2
    return 1
  }
  if [ -z "$saved_start" ] || [ -z "$saved_node" ] || [ -z "$saved_injector" ]; then
    printf 'Recorded Skin Studio injector identity is incomplete; state was preserved.\n' >&2
    return 1
  fi
  ...
  if ! recorded_injector_process_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port"; then
    ...
    # forget; a live PID with mismatched or unavailable command metadata is
    # never signalled.
  ...
  if recorded_injector_process_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port"; then
    printf 'Could not stop the recorded Skin Studio injector (PID %s).\n' "$pid" >&2
    return 1
  fi
  return 0
}

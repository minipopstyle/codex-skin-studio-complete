...
if [ "$REMOVED" = "true" ]; then
  printf 'Codex Skin Studio paused (skin removed; Codex left running). Port %s may still be in debug mode.\n' "$PORT"
elif codex_is_running; then
  printf 'Codex Skin Studio paused (injector stopped). Live remove skipped: CDP on port %s not verified.\n' "$PORT"
else
  printf 'Codex Skin Studio paused (Codex is not running).\n'
fi

...
record_start_error() {
  ...
  printf 'Codex Skin Studio: start failed at line %s (exit %s). See %s\n' "$line" "$code" "$START_ERROR_LOG" >&2
}
...
if codex_is_running && [ "$DEBUG_READY" = "false" ]; then
  if [ "$PROMPT_RESTART" = "true" ] && [ "$RESTART_EXISTING" = "false" ]; then
    /usr/bin/osascript -e 'display dialog "Codex 需要重启一次才能启用皮肤。" buttons {"取消", "重启并应用"} default button "重启并应用" with title "Codex Skin Studio"' >/dev/null \
...
if [ "$verify_code" -ne 0 ]; then
  ...
  if /usr/bin/grep -q '"installed": true' "$VERIFY_OUTPUT" 2>/dev/null; then
    printf 'Codex Skin Studio %s is active (soft verify) on port %s.\n' "$SKIN_VERSION" "$PORT"
...
printf 'Codex Skin Studio %s is active on loopback port %s.\n' "$SKIN_VERSION" "$PORT"

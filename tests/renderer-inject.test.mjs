...
assert.match(template, /homepageStyle !== "velorah"/);
assert.match(template, /homepageStyle !== "mainframe"/);
...
function createFixture(theme, {
  ...
  const context = {
    ...
    requestAnimationFrame() {
      throw new Error("scheduleEnsure must use the debounce timer, not requestAnimationFrame");
    },
